    ...
    for (const card of cards) {
        ...
        function addCorrectAnswerCheckboxHandler(checkbox, answerId) {
            ...
            checkbox.addEventListener('change', function () {
                ...
                console.log(window.MARK_CORRECT_ANSWER_URL)
                console.log(window.LIKE_OR_DISLIKE_URL_ANS)

                fetch(window.MARK_CORRECT_ANSWER_URL, {
                    method: 'POST',
                    headers: {
        ...
        function handleVote(answerId, action, likeCounter, upvoteButton, downvoteButton) {
            ...
            fetch(window.LIKE_OR_DISLIKE_URL_ANS, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRFToken": window.CSRF_TOKEN,
        ...
        function updateButtonState(button, type) {
            ...
            if (type === 'like') {
                if (action === 'like') {
                    button.src = window.UPVOTE_IMG_SRC;  
                } else {
                    button.src = window.UPVOTE_IN_IMG_SRC; 
                }
            } else if (type === 'dislike') {
                if (action === 'dislike') {
                    button.src = window.DOWNVOTE_IMG_SRC; 
                } else {
                    button.src = window.DOWNVOTE_IN_IMG_SRC; 
                }
            }
        }
    }
