...
class LoginForm(forms.Form):
    ...
    password = forms.CharField(max_length=100,min_length=8, widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': 'Password', 'style': 'margin-bottom: 7px;margin-bottom: 15px;'}))
...
class UserForm(forms.ModelForm):
    ...
    password = forms.CharField(max_length=100,min_length=8, widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': 'Password', 'style': 'margin-bottom: 7px;margin-bottom: 15px;'}))
    password_conf = forms.CharField(max_length=100,min_length=8, widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': 'Confirm Password', 'style': 'margin-bottom: 7px;margin-bottom: 15px;'}))
    ...
    def clean(self):
        ...
        if not(password_conf):
            password_conf = password_conf.lower().strip()
...
class QuestionForm(forms.ModelForm):
    title = forms.CharField(max_length=256, min_length=1, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Question title', 'style': 'margin-bottom: 7px;margin-bottom: 15px;'}))
    text = forms.CharField(min_length=1, widget=forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Your question', 'rows': 8, 'style': 'height: 300px;'}))
    tags = forms.CharField(max_length=100, min_length=1, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Tags', 'style': 'margin-bottom: 7px;margin-bottom: 15px;'}))
...
class AnswerForm(forms.ModelForm):
    text = forms.CharField(min_length=1, widget=forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Your answer', 'rows': 8, 'style': 'height: 150px;'}))
    class Meta:
        model = Answer
        fields = ['text']
