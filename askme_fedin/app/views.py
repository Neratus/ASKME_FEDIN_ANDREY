...
def tag(request, tag_name):
    ...
    tag_questions = Question.objects.get_tags(tag, user=user_profile)
...
def login(request):
    ...
    if request.user.is_authenticated:
        user_profile = get_object_or_404(Profile, user=request.user)
    return render(request, 'login.html', {'form': form, 'user_profile': user_profile})
...
def signup(request):
    ...
    return render(request, 'signup.html', {'form': form, 'user_profile': user_profile})
    ...
    if not request.user.is_authenticated:
        ...
        if request.method == 'POST':
            if form.is_valid():
                ...
                return render(request, 'ask.html', {'form': form, 'user_profile': user_profile})
        return render(request, 'ask.html', {'form': form, 'user_profile': user_profile})
...
@csrf_protect
@login_required
def like_or_dislike_answer(request):
    if not request.user.is_authenticated:
        return JsonResponse({"error": "Authentication required"}, status=401)
    if request.method == "POST":
        try:
        ...
        print(f"New rate: {answer.rate}, like_count: {answer.like_count()}, dislike_count: {answer.dislike_count()}")
